      ...
      .subscribe({
        next: (data: any) => {
          ...
          this.Datos = this.Datos.map((item: any) => ({
            ...item,
            ENVIO_MINISTERIO_BOOL: item.ENVIO_MINISTERIO === 'S'
          }));
  ...
  getBase64ImageFromURL(url: string): Promise<string> {
    ...
    async exportarConCabecera(e: any) {
      ...
      try {
        ...
        worksheet.getCell('A3').value = 'Títulos de Empleados';
        ...
        exportDataGrid({
          ...
          workbook.xlsx.writeBuffer().then((buffer: BlobPart) => {
            FileSaver.saveAs(new Blob([buffer], { type: 'application/octet-stream' }), 'TitulosEmp.xlsx');
          });
        });
